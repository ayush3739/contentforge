...
export default function SessionWorkspacePage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const [activeStage, setActiveStage] = useState<"plan" | "artifacts">(
    tabParam === "transform" ? "plan" : "artifacts"
  );
    ...
    async function load() {
      try {
        ...
        if (sess) {
          ...
          if (tabParam === "transform" || viewParam === "plan" || (sess.transformation_count ?? 0) === 0) {
            setActiveStage("plan");
        ...
        if (tabParam === "transform") setActiveStage("plan");
      }
    }
    load();
  ...
  const tabs = [
    ...
    { key: "cco", label: "CCO Semantic View", icon: Layers, badge: "v2" },
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pt-1 pb-4">
      {/* Session Top Header (Sentra & Palantir style) */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            {currentSession?.description || "Cross-platform verified artifacts generated from ingested source document"}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200">
            <button
              ...
              title="Side-by-side Source & Artifact Split Workbench (Nexora style)"
            >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
          ...
          <div className="xl:col-span-5 space-y-4">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-4">
              ...
              {/* Confidence Meter (Sentra style) */}
              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between">
                <div>
                  <span className="text-[11px] font-bold text-slate-500 uppercase">Model Grounding Integrity</span>
                  <div className="text-lg font-extrabold text-slate-900 mt-0.5">99.2% Accuracy</div>
                </div>
              ...
              {/* Claims Traceability (Nexora Extracted Fields style) */}
                ...
                <div className="bg-white rounded-2xl border border-blue-200 bg-gradient-to-r from-blue-50/60 to-indigo-50/30 p-5 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-slate-500 font-medium">Source Grounded & Ready</span>
                    </div>
                    <h3 className="text-base font-bold text-slate-900">What would you like to generate?</h3>
                    <p className="text-xs text-slate-600 mt-0.5">
                      Select target communication formats and fine-tune audience parameters to initiate verified generation.
                    </p>
                  </div>
                  <button
                    onClick={() => setActiveStage("artifacts")}
                    className="shrink-0 flex items-center gap-1.5 text-xs font-bold text-slate-700 bg-white hover:bg-slate-50 border border-slate-200 px-3.5 py-2 rounded-xl shadow-2xs transition-all"
                  >
                    View Artifacts &rarr;
                  </button>
                </div>
              ...
              <div className="space-y-4">
                <div className="bg-white rounded-2xl border border-slate-200 p-3.5 shadow-xs flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-800">Generated Intelligence Artifacts Active</span>
                  </div>
                  <button
                    onClick={() => setActiveStage("plan")}
                ...
                <ArtifactViewer artifact={mockArtifact} />
              </div>
            )}
          </div>
        </div>
            ...
            {activeTab === "overview" && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-6 rounded-2xl border border-slate-200 bg-white shadow-xs space-y-2">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Source Document</span>
                  <h3 className="text-base font-bold text-slate-900">Incident_Report.pdf</h3>
                  <p className="text-xs text-emerald-700 font-semibold flex items-center gap-1">
                    <CheckCircle2 className="h-3.5 w-3.5" /> Validated & Ingested
                  </p>
                </div>
                <div className="p-6 rounded-2xl border border-slate-200 bg-white shadow-xs space-y-2">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">CCO Semantic Version</span>
                  <h3 className="text-base font-bold text-slate-900">CCO v2 (Active)</h3>
                  <p className="text-xs text-blue-700 font-mono font-medium">4 Verified Claims • 3 Identifiers</p>
                </div>
                <div className="p-6 rounded-2xl border border-slate-200 bg-white shadow-xs space-y-2">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Artifact Generation</span>
                  <h3 className="text-base font-bold text-slate-900">5 Formats Available</h3>
                  <p className="text-xs text-emerald-700 font-semibold">99% Cryptographic Grounding</p>
                </div>
              </div>
            )}
