  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <div>
          <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-blue-600" /> 1. Select Target Output Formats
          </h3>
          <p className="text-xs text-slate-500 mt-1">Choose one or multiple communication artifacts to generate simultaneously from the CCO.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {outputCards.map((card) => {
            ...
            return (
              ...
              >
                {isSelected && <CheckCircle2 className="absolute top-4 right-4 h-5 w-5 text-blue-600" />}
                <div className={`p-3 rounded-xl w-fit mb-3 ${isSelected ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-600"}`}>
      ...
      <div className="space-y-4 pt-6 border-t border-slate-200">
        <div>
          <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider">2. Configure Transformation Parameters</h3>
          <p className="text-xs text-slate-500 mt-1">Fine-tune audience tailoring, tone, and objective filters.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6 rounded-2xl border border-slate-200 bg-white shadow-xs">
          <div>
            ...
            <select
              ...
              className="w-full rounded-xl border border-slate-200 bg-slate-50 p-2.5 text-xs text-slate-800 focus:bg-white focus:border-blue-600 focus:outline-none"
            >
          ...
          <div>
            <label className="text-xs font-semibold text-slate-700 block mb-2">Communication Tone</label>
            <select
              value={params.tone}
          ...
          <div>
            <label className="text-xs font-semibold text-slate-700 block mb-2">Detail Level</label>
            <select
          ...
          <div>
            <label className="text-xs font-semibold text-slate-700 block mb-2">Communication Objective</label>
            <select
          ...
          <div>
            <label className="text-xs font-semibold text-slate-700 block mb-2">Content Style</label>
            <select
              value={params.style}
